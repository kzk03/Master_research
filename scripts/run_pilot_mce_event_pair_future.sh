...
#
# 使い方:
#   nohup bash scripts/run_pilot_mce_event_pair_future.sh \
#       > logs/mce_pilot_event_pair_future_full.log 2>&1 &
# あるいは:
#   bash scripts/start_pilot_event_pair_future.sh
...
if [ -f "$TARGET_CACHE" ]; then
    ...
    if [ ! -f "$EXISTING_PAIR_CACHE" ]; then
        ...
        echo "  先に bash scripts/run_pilot_mce_event_comparison.sh で生成してください"
        exit 1
    fi
