...
# 使い方: bash scripts/pilot/start/start_pilot_event_dev.sh

cd "$(dirname "$0")/.."
mkdir -p logs
nohup bash scripts/pilot/run/run_pilot_mce_event_dev_comparison.sh > logs/mce_pilot_event_dev_full.log 2>&1 &
PID=$!
