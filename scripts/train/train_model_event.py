...
"""
============================================================
レビュー承諾予測 - IRL モデルの訓練 [イベント単位系列版]
============================================================
月次集約版 (train_model.py) からの変更点:
  - 1ステップ = 1レビューイベント（月次 → イベント単位）
  - スライディングウィンドウ特徴量（累積 → 直近180日）
  - イベント固有特徴量 4次元追加（state_dim=27）
  - ミニバッチ訓練（DataLoader + pad_collate）
  - 最大並列化（joblib n_jobs=-1）

【プログラムの目的】
レビュアーが将来的にレビュー依頼を承諾し続けるかどうかを予測します。
逆強化学習（IRL）を用いて、継続的にレビューを行う開発者の行動パターンから
報酬関数を学習し、それをもとに継続確率を予測します。

【予測対象】
- レビュー依頼を受けた開発者が、その依頼を承諾するかどうか
- レビュー依頼を受けていない開発者は判定対象外（除外）

【継続判定ロジック】
評価期間内の行動に基づいて判定:
1. レビュー依頼を受けていない → 除外（予測対象外）
2. レビュー依頼を受けて、少なくとも1つ承諾 → 正例（継続、label=1）
3. レビュー依頼を受けたが、全て拒否 → 負例（離脱、label=0）

【ラベルの定義】
レビュー依頼データ（build_dataset.pyで生成）:
- label = 1: レビュー依頼に14日以内に応答（承諾）
- label = 0: レビュー依頼に14日以内に応答せず（拒否/無視）

継続予測ラベル（このスクリプトで計算）:
- future_acceptance = True: 評価期間内に少なくとも1つ承諾
- future_acceptance = False: 評価期間内に依頼あり・全拒否

【データ構造】
訓練用軌跡（extract_review_acceptance_trajectories）:
{
    ...
    'sample_weight': 1.0 or 0.1                # サンプル重み
}

評価用軌跡（extract_evaluation_trajectories）:
{
    'developer_info': {...},                    # 開発者の基本情報
    'activity_history': [...],                  # 履歴期間の活動履歴
    'future_acceptance': True/False,           # 評価期間の継続ラベル
    'evaluation_period_requests': int,         # 評価期間のレビュー依頼数
    'evaluation_period_acceptances': int       # 評価期間の承諾数
}

【処理フロー】
1. レビュー依頼データを読み込み（CSV）
2. 訓練用軌跡を抽出（時系列、月次集約）
3. IRLモデルを初期化（LSTM + Focal Loss）
4. モデルを訓練
5. 最適閾値を決定（F1最大化）
6. モデルと閾値を保存
7. 評価用軌跡を抽出（スナップショット特徴量）
8. 予測を実行
9. メトリクスを計算・保存

【重要な設計】
- データリーク防止: 訓練期間内のデータのみで完結
- 時系列整合性: 各時点での過去のデータのみを使用
- クラス不均衡対策: Focal Loss + サンプル重み
- 可変長シーケンス対応: pack_padded_sequence使用
"""
...
def _process_one_reviewer_event(
    reviewer,
    df,
    history_df,
    path_extractor,
    ...
    """1レビュアー分のイベント単位軌跡を抽出する（並列処理用ワーカー関数）。

    月次版との違い:
    - 1ステップ = そのディレクトリに関するレビュー依頼1件
    - 特徴量はスライディングウィンドウ（直近 sliding_window_days 日）
    - イベント固有特徴量を追加
    - 最大 max_events 件に truncate（直近を残す）
    """
    history_start = train_start
    history_end = train_end
    ...
    for directory in reviewer_dirs:
        ...
        # ── このディレクトリに関するレビュー依頼イベントを時系列で取得 ──
        dir_events = reviewer_history[
            reviewer_history[dirs_column].map(
        ...
        for _, event_row in dir_events.iterrows():
            ...
            pf = path_extractor.compute(
                reviewer, frozenset({directory}), event_time.to_pydatetime()
            )
...
def extract_event_level_trajectories(
    ...
    1サンプル = (開発者, ディレクトリ) ペア。
    1ステップ = 1レビューイベント（月次ではない）。
    """
    from joblib import Parallel, delayed

    logger.info("=" * 80)
    logger.info("イベント単位の軌跡抽出を開始 [並列化]")
    ...
    batch_results = Parallel(n_jobs=n_jobs, backend="loky", verbose=10)(
        delayed(_process_one_reviewer_event)(
    ...
    logger.info("=" * 80)
    logger.info(f"イベント単位軌跡抽出完了: {len(trajectories)} ペア")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="イベント系列の最大長（truncation）"
    )
    parser.add_argument(
        "--sliding-window-days",
        type=int,
        default=180,
        help="特徴量計算のスライディングウィンドウ（日数）"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    if args.model is None:
        if cache_path and Path(cache_path).exists():
            ...
            # state_dim = 20 (state) + 3 (path) + 4 (event) = 27
            state_dim = 27
