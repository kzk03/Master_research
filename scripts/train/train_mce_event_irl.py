...
"""
============================================================
レビュー承諾予測 - MCE-IRL 訓練 (イベント単位版)
============================================================
train_mce_irl.py をイベント単位用に複製したバリアント。

月次版との主な違い:
  - 1 ステップ = 1 レビュー依頼 (月集約しない)
  - 軌跡は extract_event_level_trajectories で構築済みのキャッシュを使う
  - state_dim = 27 (state 20 + path 3 + event 4)
  - max_events = 256 で truncate (キャッシュ作成時に決定)
  - モデル保存名: mce_event_irl_model.pt
  - 既存軌跡抽出 (--directory-level モード) は無効化し、必ず
    --trajectories-cache 経由でキャッシュを読み込む形を推奨する

【データ構造】
訓練用軌跡 (extract_mce_event_trajectories の出力):
{
    'developer_info': {...},
    'directory': str,
    'monthly_activity_histories': [[...], ...],   # 各イベント時点までの履歴
    'step_labels': [0, 1, 1, 0, ...],             # MCE 学習で a_t として再解釈
    'step_actions': [0, 1, 1, 0, ...],            # step_labels と同義 (二値化済み)
    'event_features': [{...}, ...],               # event 4 特徴量
    'path_features_per_step': [np.ndarray, ...],
    'sample_weight': 1.0 or 0.5
}
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="レビュー承諾予測 - MCE-IRL 学習 (イベント単位版)"
    )
    parser.add_argument(
        "--reviews",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=180,
        help="特徴量計算用のスライディングウィンドウ (日数, イベント抽出時のみ有効)"
    )
    parser.add_argument(
        "--max-events",
        type=int,
        default=256,
        help="軌跡 1 ペアあたりの最大イベント数 (truncate)"
    )
    ...
    def _infer_event_state_dim(trajectories):
        """軌跡から state_dim を自動判定 (20 base + path 3 + event 4)。"""
        base = 20
        if not trajectories:
            return base
    ...
    if args.model is None:
        if cache_path and Path(cache_path).exists():
            ...
            logger.info(
                f"キャッシュから {len(train_trajectories)} 軌跡を読み込みました"
            )

            # 月次版や Focal-supervised 用キャッシュを誤って読み込んでいないか検証する。
            # イベント単位キャッシュは extract_mce_event_trajectories.py でのみ生成され、
            # step_actions / event_features / path_features_per_step が揃っている。
            if not train_trajectories:
                raise ValueError(
                    f"軌跡キャッシュが空です: {cache_path}\n"
                    f"extract_mce_event_trajectories.py で再生成してください。"
                )
            sample = train_trajectories[0]
            missing = [
                k for k in ("step_actions", "event_features",
                            "path_features_per_step",
                            "monthly_activity_histories")
                if k not in sample
            ]
            if missing:
                raise ValueError(
                    ...
                    f" 欠損キー: {missing}\n"
                    f"  （月次版 mce_traj_*.pkl や traj_*.pkl を誤って指定していないか確認してください）\n"
                    f"  → extract_mce_event_trajectories.py で生成された "
                    f"  outputs/mce_event_irl_trajectory_cache/mce_event_traj_*.pkl を指定してください。"
                )
            ef0 = sample.get("event_features") or []
            if not ef0:
                raise ValueError(
                    f"軌跡キャッシュ {cache_path} に event_features が空です。"
                    f" イベント単位キャッシュではない可能性があります。"
                )
            state_dim = _infer_event_state_dim(train_trajectories)
            if state_dim != 27:
                raise ValueError(
                    f"イベント単位 MCE-IRL は state_dim=27 を期待しますが、"
                    f" キャッシュから state_dim={state_dim} と判定されました。"
                    f" キャッシュが破損しているか、別バージョンの軌跡を指している可能性があります。"
                )
            logger.info(
            ...
            # ── イベント単位版: extract_event_level_trajectories を呼ぶ ──
            from train_model_event import extract_event_level_trajectories
            sliding_days = getattr(args, "sliding_window_days", 180)
            max_events = getattr(args, "max_events", 256)
            logger.info(
                f"イベント単位軌跡を抽出 (sliding_window={sliding_days}d, "
                f"max_events={max_events})"
            )
            train_trajectories = extract_event_level_trajectories(
                df,
                ...
                sliding_window_days=sliding_days,
                max_events=max_events,
                n_jobs=-1,
            )
            # step_actions を付与 (キャッシュ作成と同じ規約)
            for traj in train_trajectories:
                traj["step_actions"] = [
                    int(bool(l)) for l in traj.get("step_labels", [])
                ]
            state_dim = _infer_event_state_dim(train_trajectories)
            logger.info(f"state_dim 推定: {state_dim}")
        ...
        for traj in train_trajectories:
            ...
            if args.directory_level and traj.get('monthly_activity_histories'):
                ...
                # イベント単位版では event_features も渡して state_dim=27 を再構成する
        ...
        loaded_state_dim = state_encoder_w.shape[1] if state_encoder_w is not None else 27
        config = {
