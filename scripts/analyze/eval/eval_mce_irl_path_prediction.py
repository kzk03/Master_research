"""
パスごとの貢献者数予測評価スクリプト (MCE-IRL 版)

eval_path_prediction.py を MCE-IRL モデル (mce_irl_model.pt) 用に複製した版。
BatchContinuationPredictor の代わりに MCEBatchContinuationPredictor を使い、
継続確率は π(a=1|s) = softmax(R(s,·))[1] として算出される。

使い方:
    python scripts/analyze/eval/eval_mce_irl_path_prediction.py \
        --data data/combined_raw.csv \
        --raw-json data/raw_json/openstack__nova.json \
        --irl-model outputs/mce_irl/train_0-3m/mce_irl_model.pt \
        --irl-dir-model outputs/mce_irl/train_0-3m/mce_irl_model.pt \
...
from review_predictor.IRL.model.mce_irl_batch_predictor import (
    MCEBatchContinuationPredictor as BatchContinuationPredictor,
)
