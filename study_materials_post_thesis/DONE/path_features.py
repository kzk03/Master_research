...
def file_to_dir(file_path: str, depth: int = 2) -> str:
    ...
    """
    parts = [p for p in file_path.split("/") if p]# 空文字を除去（例: "/COMMIT_MSG" の先頭の空部分）から文字はfalseになるため
    if len(parts) <= 1:
        return "."
...
def load_change_dir_map(
    ...
    for change in changes:
        ...
        files: Set[str] = set()
        for rev in (change.get("revisions") or {}).values(): # revisions が None の場合は空 dict として扱うvalueしか見ない
            for fp in (rev.get("files") or {}).keys():
                if fp.startswith("/"): # ファイル名が / で始まるものは通常のファイルパスではないとみなす（例: "/COMMIT_MSG"）
                    continue
                files.add(fp)
