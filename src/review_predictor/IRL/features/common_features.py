...
_NORM_CAPS: Dict[str, float] = {
    'window_tenure_days':  180.0,  # 学習窓サイズ（180日）に合わせる。旧: 2500 は窓最大179日に対してミスマッチ
    ...
    'recent_load_ratio_30d_all':         3.0,  # 3倍でキャップ（平均の3倍以上は最大負荷。キャップなしだとmax=38.8）
    'recent_load_ratio_7d_30d':          3.0,  # 3倍でキャップ（直近7日が30日平均の3倍以上は最大負荷）
    'core_reviewer_ratio': 0.2,  # 20%でキャップ
}

# 対数変換する特徴量（右裾が重いカウント系）
# log1p(x) / log1p(cap) で 0〜1 に正規化する
_LOG1P_FEATURES: Dict[str, float] = {
    'total_changes':  500.0,  # log1p(500) ≈ 6.22 で正規化
    'total_reviews':  500.0,  # log1p(500) ≈ 6.22 で正規化
}
