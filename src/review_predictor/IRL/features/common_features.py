...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
■ 特徴量の構成（合計 26 次元: state 21 + action 5）
■ path 込みなら 31 次元 (state 21 + path 5 + action 5)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
状態特徴量（21次元）: 開発者の「現在の状態」を表す
    ...
    n_projects, cross_project_review_share, same_domain_share
...
■ 2026-05-14 改訂 (IRL vs RF AUC ギャップ解消用)
  - 追加: n_projects (Vasilescu CHI 2015 / 自前 A-2 finding)
          cross_project_review_share (Vasilescu CHI 2015)
          same_domain_share (Baysal EMSE 2016)
  - 設計方針: LSTM が累積系列から暗黙学習可能な「差分系」(delta, burst, WRC) は
              冗長になるため追加しない。
              WRC (Hannebauer ASE 2016) は初期検討したが、smoke test で任意の
              half-life で既存 count 系特徴量 (total_reviews / recent_30d_count)
              と r ≥ 0.89 の高相関を確認したため除外。LSTM が単体では作れない
              外部集約情報 (cross-project, organization affiliation) のみ追加。
...
STATE_FEATURES = [
    ...
    'n_projects',                       # 窓内に dev が活動したユニーク project 数 (Vasilescu CHI 2015)
    'cross_project_review_share',       # ホームプロジェクト外でのレビュー比率 (Vasilescu CHI 2015)
    'same_domain_share',                # owner と email domain が一致する依頼の比率 (Baysal EMSE 2016)
]
...
_NORM_CAPS: Dict[str, float] = {
    ...
    'n_projects':           5.0,  # 5 プロジェクトで 1.0 (A-2: 3+ で効果確認、5 で saturate)
}
...
def extract_common_features(
    ...
    if len(dev_data) == 0:
        ...
        # n_projects: dev_data が空でも owner_data の project から計算可能
        if 'project' in owner_data.columns and len(owner_data) > 0:
            n_projects_early = float(owner_data['project'].dropna().nunique())
        else:
            n_projects_early = 0.0
        features = {
            ...
            'n_projects':                 n_projects_early,
            'cross_project_review_share': 0.0,  # レビュー履歴なし = 計算不能
            'same_domain_share':          0.5,  # レビュー履歴なし = 中立値
    ...
    # ─── 2026-05-14 追加: 外部情報を符号化する 4 特徴量 ────────────────────
    # LSTM が累積系列から暗黙学習可能な「差分系」(delta, burst) は意図的に除外。
    # LSTM 単体では作れない「プロジェクト横断・時間減衰積分・組織情報」を符号化。

    # 20. n_projects: 窓内に dev が活動したユニーク project 数（reviewer + owner 統合）
    #     A-2: 3+ プロジェクト参加者は IRL AUC +0.019、承諾率 +8pt。
    #     Vasilescu et al. CHI 2015 / MSR 2015 の "multi-project membership"。
    if 'project' in dev_data.columns or 'project' in owner_data.columns:
        proj_set = set()
        if 'project' in dev_data.columns:
            proj_set.update(dev_data['project'].dropna().unique())
        if 'project' in owner_data.columns:
            proj_set.update(owner_data['project'].dropna().unique())
        n_projects = float(len(proj_set))
    else:
        n_projects = 0.0

    # 21. cross_project_review_share: ホームプロジェクト外でのレビュー比率
    #     home_project = dev の活動が最多の project（reviewer + owner 合算）。
    #     その外で review した割合 = レビュアーのプロジェクト横断度。
    #     Vasilescu et al. CHI 2015 の cross-project participation 概念。
    if 'project' in dev_data.columns and len(dev_data) > 0:
        # home = reviewer + owner 合算 mode
        proj_series_parts = [dev_data['project'].dropna()]
        if 'project' in owner_data.columns and len(owner_data) > 0:
            proj_series_parts.append(owner_data['project'].dropna())
        proj_all = pd.concat(proj_series_parts) if proj_series_parts else pd.Series(dtype=str)
        if len(proj_all) > 0:
            home_project = proj_all.value_counts().idxmax()
            non_home = (dev_data['project'] != home_project).sum()
            cross_project_review_share = float(non_home) / float(len(dev_data))
        else:
            cross_project_review_share = 0.0
    else:
        cross_project_review_share = 0.0

    # 22. same_domain_share: owner email domain と reviewer email domain の一致率
    #     Baysal et al. EMSE 2016: 同一企業内 PR は accept されやすい (non-technical factor)。
    #     email を 'user@domain' で split し、domain (lowercase) の一致を集計。
    if 'owner_email' in dev_data.columns and len(dev_data) > 0:
        rev_domain = email.split('@')[-1].lower() if '@' in email else ''
        owner_doms = (
            dev_data['owner_email']
            .dropna()
            .astype(str)
            .str.split('@')
            .str[-1]
        ...
        if len(owner_doms) > 0 and rev_domain:
            same_domain_share = float((owner_doms == rev_domain).mean())
        else:
            same_domain_share = 0.5  # 計算不能 = 中立値
    else:
        same_domain_share = 0.5
    ...
    features = {
        ...
        'cross_project_review_share': cross_project_review_share,
        'same_domain_share':          same_domain_share,
        # 行動特徴量（5次元）
...
def _get_default_features() -> Dict[str, float]:
    ...
    return {
        ...
        'reviewer_owner_ratio':       0.5,   # 両方ゼロ = 不明 → 中立値
        'n_projects':                 0.0,   # 活動なし
        'cross_project_review_share': 0.0,   # 計算不能
        'same_domain_share':          0.5,   # 計算不能 = 中立値
        # 行動特徴量
